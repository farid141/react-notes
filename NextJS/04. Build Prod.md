...
Setiap page dalam aplikasi akan dirender jadi `HTML` saat proses build, perlu setting `output: 'export'` di next config.
...
Pada saat build, NextJS akan melakukan fetch data ke semua server component yang pakai fetch dan disimpan dalam cache. Hasil build akan selamanya menggunakan cache tersebut. Karena pada dasarnya next js mengutamakan SEO dan konten dianggap jarang berubah. Namun jika ingin menghilangkan behavior ini ada beberapa cara:
